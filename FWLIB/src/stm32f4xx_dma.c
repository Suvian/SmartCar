/**
 ...
 RCC_AHB1PeriphResetCmd(RCC_AHB1Periph_DMA1, ENABLE) function for DMA1 or using
 RCC_AHB1PeriphResetCmd(RCC_AHB1Periph_DMA2, ENABLE) function for DMA2.
 ...
 following parameters: Source and Destination addresses, Transfer Direction,
 Transfer size, Source and Destination data formats, Circular or Normal mode,
 Stream Priority level, Source and Destination Incrementation mode, FIFO mode
 and its Threshold (if needed), Burst mode for Source and/or Destination (if
 needed) using the DMA_Init() function. To avoid filling unneccessary fields,
 you can call DMA_StructInit() function to initialize a given structure with
 default values (reset values), the modify only necessary fields (ie. Source and
 Destination addresses, Transfer size and Data Formats).

      (#) Enable the NVIC and the corresponding interrupt(s) using the function
          DMA_ITConfig() if you need to use DMA interrupts.

      (#) Optionally, if the Circular mode is enabled, you can use the Double
 buffer mode by configuring the second Memory address and the first Memory to be
 used through the function DMA_DoubleBufferModeConfig(). Then enable the Double
 buffer mode through the function DMA_DoubleBufferModeCmd(). These operations
 ...
 peripheral driver (ie. SPI_DMACmd for SPI peripheral). Once the Stream is
 enabled, it is not possible to modify its configuration unless the stream is
 stopped and disabled. After enabling the Stream, it is advised to monitor the
 EN bit status using the function DMA_GetCmdStatus(). In case of configuration
 errors or bus errors this bit will remain reset and all transfers on this
 Stream will remain on hold.
 ...
 DMA_SetCurrDataCounter(). And you can get the number of remaining data to be
 transferred using the function DMA_GetCurrDataCounter() at run time (when the
 DMA Stream is enabled and running).
 ...
 initialization phase and DMA_GetITStatus() function into interrupt routines in
             communication phase.
    [..]
 ...
 can modify the Memory Addresses using the function DMA_MemoryTargetConfig().
 Make sure that the Memory Address to be modified is not the one currently in
 use by DMA Stream. This condition can be monitored using the function
 ...
 registers and the number of remaining data will be preserved. When calling
 again this function to re-enable the Stream, the transfer will be resumed from
          the point where it was paused.

      -@- Memory-to-Memory transfer is possible by setting the address of the
 memory into the Peripheral registers. In this mode, Circular mode and Double
 Buffer mode are not allowed.
...
described in reference manual (RM0090) except the first point: waiting on EN bit
to be reset. This condition should be checked by user application using the
function DMA_GetCmdStatus() before calling the DMA_Init() function.
 ...
 * Stream is actually disabled using the function DMA_GetCmdStatus().
 * @param  DMAy_Streamx: where y can be 1 or 2 to select the DMA and x can be 0
 *         to 7 to select the DMA Stream.
...
void DMA_Init(DMA_Stream_TypeDef* DMAy_Streamx, DMA_InitTypeDef* DMA_InitStruct)
{
  ...
  tmpreg &=
    ((uint32_t) ~(DMA_SxCR_CHSEL | DMA_SxCR_MBURST | DMA_SxCR_PBURST
 ...
 * the stream, it is recommended to check (or wait until) the DMA Stream is effectively
 * enabled. A Stream may remain disabled if a configuration parameter is wrong.
 ...
 * for the next transactions (Peripheral or Memory).
 *
 ...
 * controller is the DMA controller.
 *            @arg DMA_FlowCtrl_Peripheral: DMAy_Streamx transactions flow
 * controller is the peripheral.
 * @retval None
 */
...
which the DMA controller will access alternatively (after completing transfer
to/from target memory 0, it will start transfer to/from target memory 1). This
allows to reduce software overhead for double buffering and reduce the CPU access
time.
...
address and the first Memory target from/to which the transfer will start after
...
when the DMA Stream is stopped) to modify on of the target Memories addresses or
to check wich Memory target is currently used:
...
vice versa). To check this condition, it is recommended to use the function
DMA_GetCurrentMemoryTarget() which returns the index of the Memory target
currently in use by the DMA Stream.
 ...
 * Stream is currently transferring to/from Memory 1, then it not possible to modify
 * Base address of Memory 1, but it is possible to modify Base address of Memory
 * 0. To know which Memory is currently used, you can use the function
 *          DMA_GetCurrentMemoryTarget().
 *
 * @retval None
 */
...
void DMA_PeriphAdrConfig(DMA_Stream_TypeDef* DMAy_Streamx, uint32_t PeriphBaseAddr)
{
  assert_param(IS_DMA_ALL_PERIPH(DMAy_Streamx));
  DMAy_Streamx->PAR = PeriphBaseAddr;
}
/**
 ...
 * Stream is effectively enabled. A Stream may remain disabled if a
 * configuration parameter is wrong. After disabling a DMA Stream, it is also
 * recommended to check (or wait until) the DMA Stream is effectively disabled.
 * If a Stream is disabled while a data transfer is ongoing, the current data
 * will be transferred and the Stream will be effectively disabled only after
...
// uint32_t DMA_GetCRStatus(DMA_Stream_TypeDef* DMAy_Streamx){
//   return DMAy_Streamx->CR;
// }
/**
 * @}
 */
